...
export class ProcessoVisualizarComponent implements OnInit, OnDestroy {
  ...
  private buscarRota(): void {
    this.activatedRoute.paramMap.subscribe((valor) => {
      ...
      this.consultarProcesso$(Number(id), true, false);
    });
  }

  private consultarProcesso$(id: number, proprio: boolean, pai: boolean): void {
  ...
  private agruparDocumentos(
    ...
    documentos.forEach((documento) => {
      ...
      this.listaDocumentosProcesso.push(documentoProcesso);
    });
    ...
    despachos.forEach((despacho) => {
      ...
      this.listaDocumentosProcesso.push(despachoProcesso);
    });
    ...
    anexos.forEach((anexo) => {
      ...
      this.listaDocumentosProcesso.push(anexoProcesso);
    });
  }
        ...
        next: (resposta: HttpResponse<IEntidade>) => {
          ...
          if (primeiraChamada) {
            ...
            if (this.processo.processoPai) {
              this.consultarProcesso$(this.processo.processoPai, false, true);
            }
          }
  ...
  private consultarFilhos(processosFilhosIds: number[]): void {
    ...
    if (this.processosJuntados.length === 0) this.ordenarDocumentos();
  }

  private ordenarDocumentos(): void {
    this.listaDocumentosProcesso = this.listaDocumentosProcesso.sort((a, b) => {
  ...
  protected abrirModalSignatarios(): void {
    ...
    this.ref.onClose.subscribe((signatario) => {
      if (signatario) this.adicionarSignatario(signatario);
    });
  }
  ...
  private atualizarListas(): void {
    ...
    this.consultarProcesso$(this.processo.id!, true, false);
  }
